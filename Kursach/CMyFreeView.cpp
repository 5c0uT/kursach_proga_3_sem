// CMyFreeView.cpp: файл реализации
//
...
// Обработчики сообщений CMyFreeView
...
void CMyFreeView::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	...
	else if (rc1.PtInRect(point) && pDoc->Mufta_Check != false) {
		tree.SelectItem(m_hVal);
		CObjectVal dlg;
		dlg.DoModal();
	}
	else if (rc2.PtInRect(point) && pDoc->Mufta_Check != false) {
		tree.SelectItem(m_hShponka);
		CObjectShponka dlg;
		dlg.DoModal();
	}
