// CObjectShponka.cpp: файл реализации
//
...
CObjectShponka::CObjectShponka(CWnd* pParent /*=nullptr*/)
...
// Обработчики сообщений CObjectShponka
...
{
	// Получаем значение b из документа
	...
	double bValue = pDoc->b; // Получаем значение b из документа

	// Находим все строки где ShponkaVar[row][0] == bValue
	for (int row = 0; row < 11; row++) {
		if (abs(ShponkaVar[row][0] - bValue) < 0.001) {
			// Добавляем соответствующие значения в комбобоксы
			CString hValue, wValue;

			// Добавляем высоту в комбобокс H
			hValue.Format(_T("%.1f мм"), ShponkaVar[row][1]);
			ShponkaH.AddString(hValue);

			// Добавляем ширину в комбобокс W
			wValue.Format(_T("%.1f мм"), ShponkaVar[row][2]);
			ShponkaW.AddString(wValue);
		}
	}

	// Устанавливаем первые элементы как выбранные
