		...
		{
			{
				...
				// Ищем ребро на верхней части резьбового отверстия
				// Предполагаем, что верхнее ребро имеет определенные характеристики

				int edgeCount = allEdgesAfterThread->GetCount();
				for (int i = 0; i < edgeCount; i++)
				{
					ksEntityPtr edge = allEdgesAfterThread->GetByIndex(i);
					ksEdgeDefinitionPtr edgeDef = edge->GetDefinition();

					// Проверяем принадлежность операции
					if (edgeDef->GetOwnerEntity() == pExtrudeThreadedHole)
					{
						// Проверяем тип ребра (возможно, это окружность)
						// Получаем смежные грани
						ksFaceDefinitionPtr face1 = edgeDef->GetAdjacentFace(true);
						ksFaceDefinitionPtr face2 = edgeDef->GetAdjacentFace(false);

						// Проверяем, является ли одна из граней плоской
						// (верхняя грань обычно плоская)
						if (face1)
						{
							// Пытаемся определить, что это верхняя грань по другим признакам
							// Например, по нормали или по связи с другими элементами

							// Добавляем ребро и выходим
